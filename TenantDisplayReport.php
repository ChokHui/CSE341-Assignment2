<?php	
	...
	require ('dbconfig.php');
		...
		// Remove any non-numeric characters from the form number
		$reportNo = preg_replace('/[^0-9]/', '', $reportNo);
		
		if (empty($reportNo)) {
			header("Location: TenantIndex.php?error=Form Number is required");
			exit();
		} else {
...
?>
				...
				<meta charset="UTF-8" />
					...
					<link rel="stylesheet" type="text/css" href="./styles/TenantDisplayReportComponents/hero.css">
						...
						<section class="hero container">
							<form class="hero__form" method="post" action="TenantSubmitFeedback.php" enctype="multipart/form-data">
								...
								<!-- <div class="hero__full">
									...
									<p class="hero__input hero__details"><?php echo $row["details"]; ?></p>
								</div> -->
								<div class="hero__full">
									<p class="hero__subtitle" >Details of Feedback:</p>
								...
								<div class="hero__full hero__btn-wrapper">
									<button class="hero__btn btn" type="button" onclick="location.href='TenantUpdateStatus.php?reportNo=<?php echo $row['reportNo']; ?>'">Update Status</button>
									<button class="hero__btn btn" type="button" onclick="location.href='TenantIndex.php';">Back</button>
								</div>
							</form>
							<script>
								function change(element) {
									element.classList.toggle ("hero__img-fullsize");
								}
							</script>
						</section>
					</main>
					<?php
						// require_once ('AdminUpdateStatus.php');
					?>
				</body>
				</html>
